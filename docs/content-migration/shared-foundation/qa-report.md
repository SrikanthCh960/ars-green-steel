...
- Only a build with `NEXT_PUBLIC_SITE_URL=https://arsgroup.in` emits `index, follow`, an allow-all robots policy, and the production sitemap reference.
- All other environments emit `noindex, nofollow` metadata and a disallow-all robots policy. This keeps testing and preview deployments out of search.
