...
import { calculatorBars, calculatorCities, calculatorFaqs, calculatorNotes, calculatorProducts, calculatorRegions, getRatePerKg } from "@/data/tmt-calculator";
...
export function PriceCalculatorExperience() {
  ...
  const results = useMemo(() => calculatorBars.map((bar, index) => {
    ...
    const kilograms = (projectRods / bar.piecesPerBundle) * bar.meanBundleWeight;
    const ratePerKg = getRatePerKg(region, product, bar.size);
    return { ...bar, rods: projectRods, kilograms, amount: kilograms * ratePerKg };
  }), [product, project.area, project.floors, projectReady, region]);
  ...
  function calculateProject() {
    ...
    setNotice(Object.keys(errors).length ? "Complete the project details before continuing." : "Project details saved. Your estimated requirement has been updated.");
  }
  ...
  function requestRate() {
    ...
    const params = new URLSearchParams({ source: "tmt-steel-calculator", region, city, product, quantity: String(summary.rods), weight: summary.kilograms.toFixed(2), details: "Estimated project requirement" });
  ...
  return <main className="min-h-screen overflow-x-clip bg-surface-50 text-ink-900">
    ...
    <MotionSection className="bg-surface-50 py-14 md:py-24" aria-labelledby="calculator-inputs-title">
      <div className="ars-container"><div className="mb-8"><CalculatorReveal><SectionKicker variant="brand">Calculator</SectionKicker></CalculatorReveal><CalculatorReveal delay={0.08} y={20}><h2 id="calculator-inputs-title" className="mt-5 font-display text-[clamp(2rem,3.4vw,2.5rem)] font-bold leading-[1.08]">Set your project details.</h2></CalculatorReveal><CalculatorReveal delay={0.16}><p className="mt-4 max-w-2xl text-sm leading-6 text-steel-700">Choose your region, city, product, and project details to estimate total steel and indicative cost.</p></CalculatorReveal></div>
        <div className="grid gap-8 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] xl:items-stretch">
          ...
          <section className="h-full rounded-2xl border border-ink-900/10 bg-white p-6 shadow-[0_16px_40px_rgba(6,13,30,0.06)] md:p-8" aria-labelledby="summary-title">
            ...
            <p className="mt-6 text-xs leading-5 text-steel-700">Indicative GST-inclusive amount. Delivery, transportation, loading/unloading, and the final order rate must be confirmed with ARS.</p>
