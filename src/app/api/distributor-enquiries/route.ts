...
export async function POST(request: Request) {
  ...
  if (contentLength > maximumBodyBytes) return response({ ok: false, message: "The distributor enquiry is too large." }, 413);

  let body: unknown;
  try {
    const rawBody = await request.text();
    if (Buffer.byteLength(rawBody, "utf8") > maximumBodyBytes) {
      return response({ ok: false, message: "The distributor enquiry is too large." }, 413);
    }
  ...
  const validation = validateDistributorEnquiry(body);
  if (!validation.ok) {
    return response({ ok: false, message: "Please correct the highlighted fields.", errors: validation.errors }, 400);
  }
  if (validation.isHoneypot) return response({ ok: true, message: "Your distributor enquiry has been received." }, 200);

  const now = Date.now();
  cleanupSubmissionCache(now);
  const submissionKey = createHash("sha256").update(validation.submissionId).digest("hex");
  if (recentSubmissions.has(submissionKey)) {
    return response({ ok: false, message: "This distributor enquiry is already being processed." }, 409);
  }
  ...
  try {
    ...
    return response({
      ...
      message: isConfigurationError
        ? "Distributor enquiry service is being configured. Please call ARS sales or try again later."
        : "We could not send your distributor enquiry. Please try again or call ARS sales.",
    }, 503);
  }
}
