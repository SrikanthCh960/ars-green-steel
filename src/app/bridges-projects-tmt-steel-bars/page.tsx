...
const certifications = [
  ["BIS Certified – Manufactured in compliance with IS 1786 standards.", "/ars-assets/certifications/previews/tmt-renewal.png"],
  ["ISO 9001 Certified – Quality management systems for consistent manufacturing.", "/ars-assets/certifications/previews/iso-9001.png"],
  ["ISO 14001 Certified – Environmentally responsible manufacturing practices.", "/ars-assets/certifications/previews/iso-14001.png"],
  ["SGS Tested – Independently verified product quality and performance.", "/ars-assets/awards-certificates-img3.png"],
  ["GreenPro Certified – Recognised for sustainable manufacturing.", "/ars-assets/blog-banners/greenpro-certified-steel-the-future-of-sustainable-construction/GreenPro-Certified-Steel-The-Future-of-Sustainable-Construction.jpeg"],
  ["Environmental Product Declaration (EPD) – Transparent, third-party verified environmental performance.", "/ars-assets/certifications/previews/epd-certificate.png"],
  ["SGBC 4-Ticks Leader Rating – Supporting sustainable infrastructure and green building initiatives.", "/ars-assets/certifications/previews/sgbc-certificate.png"],
  ["Government of Tamil Nadu PWD Approved – Approved for use in public infrastructure and construction projects.", "/ars-assets/certifications/previews/pwd-approval.png"],
] as const;
  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-surface-50 text-ink-900">
      ...
      <MotionSection className="bg-bg-dark py-20 text-white md:py-28">
        <div className="ars-container"><SectionKicker variant="light">Our products</SectionKicker><div className="grid gap-8 lg:grid-cols-[0.48fr_0.52fr] lg:items-end"><h2 className="section-title section-title-light">TMT Bars Built for Bridge &amp; Infrastructure Construction</h2><p className="text-[15px] leading-7 text-white/70">Choose the right reinforcement solution based on structural requirements, environmental conditions, and long-term durability for bridge and infrastructure projects.</p></div><div className="mt-12 grid gap-5 md:grid-cols-2"><Link href="/products/ars-550d" className="focus-ring group border border-white/20 bg-white/[0.04] p-7 transition hover:border-white/45 hover:bg-white/[0.09]"><p className="font-technical text-xs font-bold tracking-[0.2em] text-white/50">ARS TMT BARS</p><h3 className="mt-7 font-display text-3xl font-bold text-white">ARS Fe550D</h3><p className="mt-4 max-w-md text-sm leading-7 text-white/70">General bridge &amp; infrastructure projects</p><span className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-white">Explore ARS Fe550D <ArrowRight aria-hidden="true" size={15}/></span></Link><Link href="/products/ars-crs-550d" className="focus-ring group border border-white/20 bg-white/[0.04] p-7 transition hover:border-white/45 hover:bg-white/[0.09]"><p className="font-technical text-xs font-bold tracking-[0.2em] text-white/50">ARS TMT BARS</p><h3 className="mt-7 font-display text-3xl font-bold text-white">ARS CRS 550D</h3><p className="mt-4 max-w-md text-sm leading-7 text-white/70">Coastal, high-moisture &amp; high salinity infrastructure</p><span className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-white">Explore ARS CRS 550D <ArrowRight aria-hidden="true" size={15}/></span></Link></div><div className="mt-5 overflow-x-auto border border-white/16"><table className="min-w-[680px] w-full text-left"><caption className="sr-only">Comparison of ARS Fe550D and CRS 550D TMT Bars for bridge and infrastructure construction</caption><thead className="bg-white/10 text-sm"><tr><th scope="col" className="p-5 font-technical uppercase tracking-[0.14em] text-white/65">Feature</th><th scope="col" className="p-5 font-display text-lg text-white">ARS Fe550D</th><th scope="col" className="p-5 font-display text-lg text-white">ARS CRS 550D</th></tr></thead><tbody>{productRows.map(([feature, fe550d, crs]) => <tr key={feature} className="border-t border-white/12"><th scope="row" className="p-5 text-sm font-bold text-white">{feature}</th><td className="p-5 text-sm leading-6 text-white/72">{fe550d}</td><td className="p-5 text-sm leading-6 text-white/72">{crs}</td></tr>)}</tbody></table></div></div>
      </MotionSection>
      ...
      <MotionSection className="bg-white py-20 md:py-28"><div className="ars-container grid gap-12 lg:grid-cols-[0.38fr_0.62fr]"><div><SectionKicker>FAQs</SectionKicker><h2 className="section-title">Technical guidance for bridge and infrastructure projects</h2></div><div className="divide-y divide-ink-900/10 border-y border-ink-900/10">{faqs.map(([question, answer]) => <details key={question} className="group py-5"><summary className="focus-ring cursor-pointer list-none pr-8 font-display text-lg font-bold leading-7 text-ink-900 marker:content-none">{question}<span aria-hidden="true" className="float-right text-brand-red transition group-open:rotate-45">+</span></summary><p className="max-w-3xl pt-4 text-[15px] leading-7 text-steel-700">{answer}</p></details>)}</div></div></MotionSection>

      <MotionSection className="bg-surface-50 py-10"><div className="ars-container"><p className="font-technical text-xs font-bold uppercase tracking-[0.2em] text-steel-700">Continue your project review</p><nav aria-label="Related bridge and flyover project resources" className="mt-5 flex flex-wrap gap-x-6 gap-y-4"><Link href="/products" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-brand-blue">Products <ArrowRight aria-hidden="true" size={15}/></Link><Link href="/certifications" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-brand-blue">Quality &amp; certifications <ArrowRight aria-hidden="true" size={15}/></Link><Link href="/dealer-locator" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-brand-blue">Find a dealer <ArrowRight aria-hidden="true" size={15}/></Link><Link href="/tmt-calculator" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-brand-blue">TMT calculator <ArrowRight aria-hidden="true" size={15}/></Link><Link href="/road-projects-tmt-steel-bars" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-brand-blue">Road projects <ArrowRight aria-hidden="true" size={15}/></Link><Link href="/institutions-projects-tmt-steel-bars" className="focus-ring inline-flex items-center gap-2 text-sm font-bold text-brand-blue">Institutional projects <ArrowRight aria-hidden="true" size={15}/></Link></nav></div></MotionSection>

      <ContactCta eyebrow="Infrastructure projects" headline="Build Infrastructure That Stands the Test of Time" body="Whether you're planning a bridge, flyover, or large-scale infrastructure project, ARS TMT Bars deliver the strength, quality, and reliability your structures depend on." primaryLabel="Contact Our Team" primaryHref="/request-quote" secondaryLabel="Find a dealer" secondaryHref="/dealer-locator" />
    </main>
  );
}
