...
const projectRequirements = [
  ...
  ["Supports Safer Infrastructure", "Reliable reinforcement contributes to stronger retaining walls, culverts, bridges, drains, and other critical structures that support every road network.", ShieldCheck],
  ["Built for Sustainable Development", "Long-lasting infrastructure reduces repair cycles, conserves resources, and supports more sustainable road construction for the future.", Factory],
] as const;
...
const products = [
  ["ARS Fe550D TMT Bars", "For highways, bridges, flyovers, retaining walls, and reinforced concrete structures that demand high strength and excellent ductility.", "/products/ars-550d", "Explore Fe550D"],
  ["ARS CRS 550D TMT Bars", "Designed for road infrastructure exposed to high moisture, water crossings, and high-salinity environments where enhanced corrosion resistance is essential.", "/products/ars-crs-550d", "Explore CRS 550D"],
...
const qualityEvidence = [
  ...
  ["ISO Certified Manufacturing", "/ars-assets/certifications/previews/iso-9001.png"],
  ...
  ["GRIHA Listed", "/ars-assets/certifications/previews/griha-certificate.png"],
  ["Precision Manufacturing & Spectrometer Testing", undefined, BadgeCheck],
] as const;
...
export default function RoadProjectsPage() {
  return (
    <main className="min-h-screen overflow-x-clip bg-surface-50 text-ink-900">
      ...
      <MotionSection className="bg-white py-20 md:py-28"><div className="ars-container grid gap-12 lg:grid-cols-[0.42fr_0.58fr] lg:gap-20"><div><SectionKicker>Road engineering</SectionKicker><h2 className="section-title">Every Road Is Only as Strong as What Lies Beneath.</h2></div><div><p className="max-w-3xl text-lg leading-9 text-steel-700">Roads are more than asphalt. Beneath every highway, expressway, and urban road network lies a system of reinforced concrete structures that carry traffic, withstand environmental conditions, and support decades of continuous use. Choosing high-quality TMT bars is essential to building road infrastructure that delivers lasting strength, durability, and safety.</p><div className="mt-10 grid gap-px overflow-hidden border border-ink-900/10 bg-ink-900/10 md:grid-cols-2 xl:grid-cols-5">{projectRequirements.map(([title, body, Icon], index) => <article key={title} className="bg-surface-50 p-6"><span className="font-technical text-xs font-bold tracking-[0.2em] text-brand-red">0{index + 1}</span><Icon aria-hidden="true" className="mt-10 text-brand-blue" size={25} /><h3 className="mt-5 font-display text-xl font-bold text-ink-900">{title}</h3><p className="mt-3 text-sm leading-6 text-steel-700">{body}</p></article>)}</div></div></div></MotionSection>
      ...
      <MotionSection className="bg-white py-20 md:py-28"><div className="ars-container grid gap-12 lg:grid-cols-[0.38fr_0.62fr]"><div><SectionKicker>Quality &amp; certifications</SectionKicker><h2 className="section-title">Every Bar Manufactured to Meet Stringent Quality Standards</h2></div><div><p className="text-[15px] leading-7 text-steel-700">Road infrastructure is expected to perform for decades under continuous traffic, changing weather conditions, and demanding load cycles. That&apos;s why every ARS TMT Bar undergoes rigorous quality testing and is manufactured in compliance with recognised national and international standards.</p><ul className="mt-10 grid gap-px overflow-hidden border border-ink-900/10 bg-ink-900/10 sm:grid-cols-2 lg:grid-cols-3">{qualityEvidence.map(([title, image, Icon]) => <li key={title} className="flex min-h-44 flex-col items-center justify-center bg-surface-50 p-5 text-center"><span className="relative flex h-16 w-28 items-center justify-center overflow-hidden">{image ? <Image src={image} alt="" fill sizes="112px" className="object-contain" /> : <Icon size={34} className="text-brand-blue" aria-hidden="true" />}</span><span className="mt-4 text-sm font-bold leading-5 text-ink-900">{title}</span></li>)}</ul><Link href="/certifications" className="focus-ring mt-8 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-brand-blue hover:text-brand-blue-dark">Explore quality &amp; certifications <ArrowRight aria-hidden="true" size={16} /></Link></div></div></MotionSection>
