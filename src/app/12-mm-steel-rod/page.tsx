  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-surface-50 text-ink-900">
      ...
      <section className="ars-page-hero relative flex min-h-[560px] items-end overflow-hidden bg-ink-950 md:min-h-[600px] lg:h-[680px] lg:min-h-[680px] lg:max-h-[680px]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(6,13,30,0.97)_0%,rgba(6,13,30,0.8)_48%,rgba(6,13,30,0.22)_100%)]" /><div className="absolute inset-x-0 bottom-0 h-2/5 bg-[linear-gradient(to_top,rgba(6,13,30,0.82),transparent)]" />
        <div className="ars-container relative z-10 w-full pb-14 md:pb-16"><div className="max-w-3xl"><SectionKicker variant="light">ARS TMT BAR SIZES</SectionKicker><h1 className="mt-5 font-display text-[clamp(2.65rem,6vw,4.5rem)] font-extrabold uppercase leading-[1] tracking-[-0.025em] text-white">ARS 12MM TMT Bar: Engineered for Exceptional Load-Bearing Strength</h1><p className="mt-5 max-w-2xl text-[15px] leading-[1.75] text-white/78">Experience superior strength and durability with ARS 12MM TMT Steel Bar, designed for high-demand structural applications like high-rise buildings and critical infrastructure.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/steel-price-today" className="focus-ring inline-flex min-h-12 items-center gap-2.5 rounded-full bg-brand-red px-6 py-3 text-[14px] font-bold text-white transition hover:opacity-90"><Calculator size={15} /> Check price</Link><Link href="/request-quote" className="focus-ring inline-flex min-h-12 items-center gap-2.5 rounded-full border border-white/35 px-6 py-3 text-[14px] font-bold text-white transition hover:bg-white/10"><FileText size={15} /> Request a quote</Link></div></div></div>
      </section>
