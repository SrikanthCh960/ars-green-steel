...
import { ProductsReveal, ProductsSectionMotion } from "@/components/products-page-motion";
...
export default function ProductsPage() {
  return (
    <main className="min-h-screen bg-surface-50 text-ink-900">
      <noscript>
        <style>{"[data-products-motion] { opacity: 1 !important; transform: none !important; }"}</style>
      </noscript>
      <SiteHeader />
      ...
      <section className="ars-page-hero relative flex min-h-[560px] items-end overflow-hidden bg-ink-950 md:min-h-[600px] lg:h-[680px] lg:min-h-[680px] lg:max-h-[680px]">
        ...
        <div className="ars-container relative z-10 w-full pb-14 pt-36 md:pb-20">
          <div className="max-w-4xl">
            ...
            <ProductsReveal delay={0.08} y={22}>
      ...
      <ProductsSectionMotion className="border-b border-surface-100 bg-white py-14">
        <div className="ars-container grid grid-cols-2 gap-8 lg:grid-cols-4 lg:gap-0 lg:divide-x lg:divide-ink-900/10">
          {trustItems.map((item, index) => <ProductsReveal key={item.value} delay={index * 0.07} y={14}><div className="flex flex-col px-0 lg:items-center lg:px-8 lg:text-center"><span className="font-display text-[clamp(1.8rem,2.5vw,2.4rem)] font-extrabold leading-none tracking-[-0.03em] text-brand-blue">{item.value}</span><span className="mb-1 mt-1.5 text-[12px] font-bold uppercase tracking-[0.06em] text-ink-900">{item.label}</span><span className="max-w-[200px] text-[12px] leading-normal text-grey-600">{item.description}</span></div></ProductsReveal>)}
        </div>
      </ProductsSectionMotion>

      <ProductsSectionMotion className="bg-brand-blue py-24" id="comparison">
        <div className="ars-container"><ProductsReveal><SectionKicker variant="light">Product Comparison</SectionKicker></ProductsReveal>
          <ProductsReveal delay={0.12}><div className="hidden overflow-hidden rounded-2xl border border-white bg-brand-blue md:block"><table className="w-full border-collapse text-left"><caption className="sr-only">Comparison of ARS Fe 550D and ARS CRS Fe 550D TMT Bars.</caption><thead className="bg-brand-blue-dark"><tr><th scope="col" className="w-[22%] border-b border-white/35 px-5 py-4 text-[11px] font-bold uppercase tracking-[0.1em] text-white">Feature</th><th scope="col" className="border-b border-white/35 px-5 py-4 text-[12px] font-bold text-white">ARS 550D</th><th scope="col" className="border-b border-white/35 px-5 py-4 text-[12px] font-bold text-white">ARS CRS 550D</th></tr></thead><tbody>{comparisonRows.map(([feature, standard, crs], index) => <tr key={feature} className={index % 2 === 0 ? "bg-brand-blue" : "bg-brand-blue-dark/70"}><th scope="row" className="border-b border-white/25 px-5 py-4 text-[12px] font-semibold uppercase tracking-[0.04em] text-white">{feature}</th><td className="border-b border-white/25 px-5 py-4 text-[13px] leading-[1.5] text-white/85">{standard}</td><td className="border-b border-white/25 px-5 py-4 text-[13px] leading-[1.5] text-white/85">{crs}</td></tr>)}</tbody></table></div>
          <dl className="divide-y divide-white/25 rounded-2xl border border-white bg-brand-blue md:hidden">{comparisonRows.map(([feature, standard, crs]) => <div key={feature} className="p-5"><dt className="text-[11px] font-bold uppercase tracking-[0.1em] text-white">{feature}</dt><dd className="mt-4 grid gap-4 sm:grid-cols-2"><div><p className="text-[11px] font-bold uppercase tracking-[0.08em] text-white/65">ARS 550D</p><p className="mt-1.5 text-[13px] leading-[1.55] text-white/85">{standard}</p></div><div><p className="text-[11px] font-bold uppercase tracking-[0.08em] text-white/65">ARS CRS 550D</p><p className="mt-1.5 text-[13px] leading-[1.55] text-white/85">{crs}</p></div></dd></div>)}</dl></ProductsReveal>
        </div>
      </ProductsSectionMotion>

      <ProductsSectionMotion className="bg-white py-24" id="sizes"><div className="ars-container"><div className="mb-14 grid items-end gap-10 lg:grid-cols-2"><div><ProductsReveal><SectionKicker variant="brand">Bar sizes</SectionKicker></ProductsReveal><ProductsReveal delay={0.08} y={22}><h2 className="font-display text-[clamp(2rem,3.4vw,2.25rem)] font-bold leading-[1.1] tracking-[-0.025em] text-ink-900">Available TMT Bar Sizes</h2></ProductsReveal></div><ProductsReveal delay={0.16}><p className="text-[15px] leading-[1.8] text-steel-700">Choose the bar diameter that matches your structural design. ARS TMT Bars are available in multiple sizes for residential, commercial, industrial, and infrastructure construction.</p></ProductsReveal></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{sizes.map(([size, description, href], index) => <ProductsReveal key={size} delay={0.24 + index * 0.06}><Link href={href} className="focus-ring group block rounded-2xl border-[1.5px] border-surface-100 bg-surface-50 p-6 transition duration-200 hover:-translate-y-0.5 hover:border-brand-blue/30 hover:bg-white hover:shadow-lg motion-reduce:transform-none motion-reduce:transition-none"><div className="mb-6 flex h-11 w-11 items-center justify-center rounded-xl bg-white text-brand-blue ring-1 ring-ink-900/[0.06] transition group-hover:bg-brand-blue group-hover:text-white motion-reduce:transition-none"><Ruler aria-hidden="true" size={20} /></div><h3 className="font-display text-[2rem] font-extrabold tracking-[-0.02em] text-ink-900">{size}</h3><p className="mt-3 text-[13px] leading-[1.6] text-grey-600">{description}</p><span className="mt-5 inline-flex items-center gap-2 text-[12px] font-bold uppercase tracking-[0.08em] text-brand-blue">View size details <ArrowRight aria-hidden="true" size={14} className="transition-transform group-hover:translate-x-1 motion-reduce:transform-none motion-reduce:transition-none" /></span></Link></ProductsReveal>)}</div></div></ProductsSectionMotion>

      <ProductsSectionMotion className="bg-white py-24" id="grades">
        <div className="ars-container"><div className="mb-14 grid items-end gap-12 lg:grid-cols-2"><div><ProductsReveal><SectionKicker variant="brand">Product Range</SectionKicker></ProductsReveal><ProductsReveal delay={0.08} y={22}><h2 className="font-display text-[clamp(2rem,3.4vw,2.25rem)] font-bold leading-[1.1] tracking-[-0.025em] text-ink-900">Explore the ARS Product Range</h2></ProductsReveal></div><ProductsReveal delay={0.16}><p className="text-[15px] leading-[1.8] text-steel-700">Explore ARS reinforcement products for everyday construction, demanding environments, and faster on-site assembly. Select a product to review its dedicated information and enquiry pathway.</p></ProductsReveal></div><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{products.map((product, index) => { const Icon = product.icon; return <ProductsReveal key={product.name} delay={0.24 + index * 0.08}><Link href={product.route} className="focus-ring group block overflow-hidden rounded-2xl border-[1.5px] border-surface-100 bg-white transition duration-200 hover:-translate-y-0.5 hover:shadow-lg motion-reduce:transform-none motion-reduce:transition-none"><div className="relative h-56 overflow-hidden bg-surface-50 p-8"><Image src={product.image} alt={`${product.name} product`} fill sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-contain p-8 transition duration-500 group-hover:scale-[1.02] motion-reduce:transform-none motion-reduce:transition-none" /></div><div className="p-7"><div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-brand-blue/[0.06] text-brand-blue"><Icon aria-hidden="true" size={20} /></div><h3 className="font-display text-[22px] font-bold text-ink-900">{product.name}</h3><p className="mt-3 text-[14px] leading-[1.7] text-steel-700">{product.description}</p><ul className="mt-5 flex flex-col gap-2">{product.points.map((point) => <li key={point} className="flex items-start gap-2.5"><CheckCircle2 aria-hidden="true" size={15} className="mt-0.5 shrink-0 text-brand-red" /><span className="text-[13px] font-medium text-steel-700">{point}</span></li>)}</ul></div></Link></ProductsReveal> })}</div></div>
      </ProductsSectionMotion>

      <ProductsSectionMotion className="bg-surface-50 py-24" id="faqs"><div className="ars-container max-w-4xl"><ProductsReveal><SectionKicker variant="brand">FAQs</SectionKicker></ProductsReveal><ProductsReveal delay={0.12}><FaqList className="mt-10" items={faqs.map(([question, answer]) => ({ question, answer }))} /></ProductsReveal></div></ProductsSectionMotion>

      <ProductsSectionMotion className="relative overflow-hidden bg-brand-blue py-20"><div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(circle at 15% 50%, rgba(222,18,26,0.7) 0%, transparent 55%)" }} /><div className="ars-container relative z-10"><div className="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center"><div className="max-w-3xl"><ProductsReveal><SectionKicker variant="light">READY TO BUILD</SectionKicker></ProductsReveal><ProductsReveal delay={0.08} y={22}><h2 className="mb-3 font-display text-[clamp(2rem,3.4vw,2.25rem)] font-bold leading-[1.12] tracking-[-0.02em] text-white">Choose the Right TMT Bar with Confidence</h2></ProductsReveal><ProductsReveal delay={0.16}><p className="text-[14px] leading-[1.7] text-white/70">Whether you&apos;re building a home, a commercial project, or infrastructure, selecting the right reinforcement steel makes all the difference. Compare ARS Fe 550D and ARS CRS Fe 550D, speak with our experts, and choose the TMT Bar that best matches your project&apos;s structural and environmental requirements.</p></ProductsReveal></div><ProductsReveal delay={0.26}><div className="flex flex-wrap gap-3"><Link href="/request-quote" className="focus-ring inline-flex min-h-11 items-center gap-2.5 rounded-full bg-brand-red px-6 py-3.5 text-[14px] font-bold text-white transition hover:opacity-90">Talk to Sales <ArrowRight aria-hidden="true" size={15} /></Link><Link href="/our-network" className="focus-ring inline-flex min-h-11 items-center gap-2.5 rounded-full border-[1.5px] border-white/30 px-6 py-3.5 text-[14px] font-semibold text-white/85 transition hover:bg-white/[0.15]">Find a Dealer <ArrowRight aria-hidden="true" size={15} /></Link></div></ProductsReveal></div></div></ProductsSectionMotion>
