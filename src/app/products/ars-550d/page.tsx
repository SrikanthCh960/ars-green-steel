...
const trustProof = [
  { title: "BIS", icon: ShieldCheck },
  { title: "Green Steel Certified", icon: Leaf },
  { title: "TN PWD approved", icon: Building2 },
  { title: "SGBP Leader (4star)", icon: Award },
  { title: "GRIHA", icon: BadgeCheck },
  { title: "EPD Verified", icon: FileText },
];

const availableSizes = ["8mm", "10mm", "12mm", "16mm", "20mm", "25mm", "32mm"];
...
const faqs = [
  ...
  [
    "What is the difference between Fe 500D and Fe550D TMT Bars?",
    "The primary difference is yield strength. Fe550D offers a minimum yield strength of 550 MPa, while Fe 500D provides 500 MPa. With higher strength and excellent ductility, Fe550D is preferred for projects that require enhanced structural performance and efficient steel utilisation.",
  ],
  [
    ...
    <main className="min-h-screen overflow-x-clip bg-surface-50 text-ink-900">
      ...
      <MotionSection className="bg-brand-blue py-20 md:py-24">
        <div className="ars-container">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{benefits.map((benefit, index) => <article key={benefit.title} className="flex min-h-[235px] flex-col overflow-hidden rounded-2xl border border-white/[0.1] bg-white/[0.05] p-6"><div className="flex items-center justify-between"><span className="flex h-10 w-10 items-center justify-center rounded-xl border border-brand-red/30 bg-brand-red/10 text-brand-red">{benefit.icon}</span><div className="relative h-11 w-20 overflow-hidden rounded-md border border-white/10 bg-white/5"><Image src="/ars-assets/TMT-Bars.png" alt="" fill sizes="80px" className="object-cover opacity-80" aria-hidden="true" /></div><span className="sr-only">{String(index + 1).padStart(2, "0")}</span></div><h3 className="mt-6 font-display text-[16px] font-bold text-white">{benefit.title}</h3><p className="mt-2 text-[13px] leading-[1.65] text-white/70">{benefit.desc}</p></article>)}</div>
        </div>
      </MotionSection>
      ...
      <MotionSection className="border-y border-brand-blue/10 bg-white py-16 md:py-20" id="sizes">
        <div className="ars-container flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <SectionKicker variant="brand">Sizes Available</SectionKicker>
            <h2 className="font-display text-[clamp(2rem,3.4vw,2.25rem)] font-bold leading-[1.1] tracking-[-0.025em] text-ink-900">Available Sizes</h2>
          </div>
          <ul className="flex flex-wrap gap-3" aria-label="ARS 550D available sizes">
            {availableSizes.map((size) => <li key={size} className="min-w-16 border border-brand-blue/20 bg-surface-50 px-4 py-3 text-center font-display text-lg font-bold text-brand-blue">{size}</li>)}
          </ul>
        </div>
      </MotionSection>

      <MotionSection className="bg-surface-50 py-20 md:py-24">
        <div className="ars-container grid items-center gap-10 lg:grid-cols-[0.9fr_1.1fr]"><div><SectionKicker variant="brand">Trust</SectionKicker><p className="mt-5 font-display text-xl font-bold text-ink-900">Tested. Certified. Trusted.</p><div className="mt-7 grid grid-cols-2 gap-3">{trustProof.map((proof) => { const Icon = proof.icon; return <div key={proof.title} className="flex min-h-24 flex-col justify-between border border-surface-100 bg-white p-4 text-brand-blue shadow-[0_12px_30px_rgba(6,13,30,0.04)]"><Icon size={21} className="text-brand-red" aria-hidden="true" /><span className="mt-4 text-[13px] font-bold leading-5">{proof.title}</span></div>; })}</div><div className="mt-7 flex flex-wrap gap-3"><span aria-disabled="true" className="inline-flex min-h-11 cursor-not-allowed items-center gap-2 rounded-full border border-brand-blue/20 px-5 py-2.5 text-[13px] font-bold text-brand-blue"><Download size={14} /> Download brochure</span><Link href="/certifications#product-statutory-approvals" className="focus-ring inline-flex min-h-11 items-center gap-2 rounded-full bg-brand-blue px-5 py-2.5 text-[13px] font-bold text-white"><Download size={14} /> Download certifications</Link></div></div><div className="relative min-h-[320px] overflow-hidden rounded-2xl bg-ink-950"><Image src="/ars-assets/CRS.png" alt="ARS CRS 550D" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" /><div className="absolute inset-0 bg-[linear-gradient(to_top,rgba(6,13,30,0.92),rgba(6,13,30,0.12))]" /><div className="absolute inset-x-6 bottom-6"><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-white/70">Corrosion resistant steel</p><Link href="/products/ars-crs-550d" className="focus-ring mt-3 inline-flex items-center gap-2 rounded-full bg-white px-5 py-2.5 text-[13px] font-bold text-brand-blue">Explore ARS CRS 550D <ArrowRight size={14} /></Link></div></div></div>
      </MotionSection>
