...
export default function ArsCrs550DPage() {
  return (
    <main className="min-h-screen overflow-x-clip bg-surface-50 text-ink-900">
      ...
      <MotionSection className="bg-brand-blue py-20 md:py-24"><div className="ars-container"><div className="mb-12 grid items-start gap-8 lg:grid-cols-2"><div><SectionKicker variant="light">WHY CHOOSE ARS CRS 550D</SectionKicker><h2 className="font-display text-[clamp(2rem,3.4vw,2.25rem)] font-bold leading-[1.1] tracking-[-0.025em] text-white">Engineered for Corrosion Resistance. Built for Generations.</h2></div><p className="text-[15px] leading-[1.8] text-white/75 lg:pt-12">Designed for structures exposed to moisture and aggressive environments, ARS CRS 550D delivers dependable structural performance with long-term corrosion protection.</p></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{benefits.map((benefit, index) => <article key={benefit.title} className="flex min-h-[235px] flex-col overflow-hidden rounded-2xl border border-white/[0.1] bg-white/[0.05] p-6"><div className="flex items-center justify-between"><span className="flex h-10 w-10 items-center justify-center rounded-xl border border-brand-red/30 bg-brand-red/10 text-brand-red">{benefit.icon}</span><div className="relative h-11 w-20 overflow-hidden rounded-md border border-white/10 bg-white/5"><Image src="/ars-assets/CRS.png" alt="" fill sizes="80px" className="object-cover opacity-80" aria-hidden="true" /></div><span className="sr-only">{String(index + 1).padStart(2, "0")}</span></div><h3 className="mt-6 font-display text-[16px] font-bold text-white">{benefit.title}</h3><p className="mt-2 text-[13px] leading-[1.65] text-white/70">{benefit.desc}</p></article>)}</div></div></MotionSection>
      ...
      <MotionSection className="border-y border-brand-blue/10 bg-white py-16 md:py-20" id="sizes"><div className="ars-container flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between"><div><SectionKicker variant="brand">Sizes Available</SectionKicker><h2 className="font-display text-[clamp(2rem,3.4vw,2.25rem)] font-bold leading-[1.1] tracking-[-0.025em] text-ink-900">Available Sizes</h2></div><ul className="flex flex-wrap gap-3" aria-label="ARS CRS 550D available sizes">{availableSizes.map((size) => <li key={size} className="min-w-16 border border-brand-blue/20 bg-surface-50 px-4 py-3 text-center font-display text-lg font-bold text-brand-blue">{size}</li>)}</ul></div></MotionSection>
