  ...
  return (
    <PageShell
      hero={{
        ...
        accent: "you enquire.",
        body: "Plan your steel requirement with a simple enquiry-first flow that helps the ARS team respond with better context.",
    ...
    >
      <ProofMetrics
        metrics={[
          ...
          { kicker: "support", value: "Sales", label: clientVerificationSummary.calculator },
      ...
      <MotionSection className="bg-white py-24">
        <div className="ars-container grid gap-8 lg:grid-cols-[0.42fr_0.58fr]">
          <SectionIntro
            ...
            title="Share project details for a better estimate."
            body="Enter the basic project information sales teams usually need before helping with quantity and quote support."
          />
          <div className="rounded-[8px] border border-ink-900/10 bg-[#f8f9fb] p-6">
            <div className="grid gap-4 md:grid-cols-2">
              {calculatorInputFields.map((field) => (
                <label key={field} className="grid gap-2 text-sm font-bold text-ink-900">
                  {field}
                  <input name={field.toLowerCase().replace(/[^a-z0-9]+/g, "-")} className="h-12 rounded-[6px] border border-ink-900/12 bg-white px-4 font-normal outline-none transition focus:border-brand-blue" placeholder="Enter details" />
                </label>
              ))}
            </div>
            <div className="mt-6 rounded-[8px] border border-brand-blue/20 bg-white p-5">
              ...
              <p className="mt-3 text-base leading-7 text-steel-700">
                Share these inputs with ARS sales to receive requirement guidance based on your project type, location, and product grade.
              </p>
            </div>
