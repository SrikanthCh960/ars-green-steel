...
| `BlogArticleTemplate` | `src/components/blog-article-template.tsx` | Shared long-form article shell for migrated ARS blog posts | Uses a wide two-column desktop reading layout with a 280px white, numbered sticky page-index card, a native mobile “On this page” disclosure, and server-rendered unique fragment targets derived from the article’s actual headings; articles without usable headings use a focused single-column layout. Includes the article-owned quick takeaway, category-specific post-reading CTA, related articles, and shared final contact band | All `/blog/[slug]` articles |
