...
| `AnalyticsInteractions` | `src/components/analytics-interactions.tsx` | Shared privacy-minimized telephone and WhatsApp click measurement for direct GA4 | Uses delegated click listeners; emits `phone_click` and `whatsapp_click` only when public analytics environment gating is enabled; sends the source page and link location without raw telephone URLs or visitor-entered data | Root layout across all routes |
