...
| `currentCareerOpenings` | `src/data/careers.ts` | Current client-supplied vacancies with stable role-and-region IDs, separate from archived career profiles | `/careers`, `/careers/apply`, and the career application endpoint | Keep role, region, and open-position counts aligned with client updates; validate IDs again on each submission |
| `getCareerApplicationsConfig` | `src/lib/career-applications-config.ts` | Server-only feature gate and configuration for career application storage | Careers pages and `/api/career-applications` | Requires existing Sheets access, narrow Google Drive OAuth credentials, a Drive folder ID, a Careers tab, and an explicit enable flag; never expose credentials to browser code |
