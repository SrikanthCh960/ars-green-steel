...
| `AnalyticsInteractions` | `src/components/analytics-interactions.tsx` | Shared privacy-minimized telephone and WhatsApp click measurement for independently controlled direct GA4 and GTM | Uses delegated click listeners; emits `phone_click` and `whatsapp_click` only to the integrations enabled through their separate public environment gates; sends the source page and link location without raw telephone URLs or visitor-entered data | Root layout across all routes |
